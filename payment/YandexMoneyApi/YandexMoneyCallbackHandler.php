...
class YandexMoneyCallbackHandler
{
     ...
     * @param $turboApi
     */
    public function __construct($turboApi)
    {
    ...
    {
        $orderId       = $this->turboApi->request->get('order');
        $order         = $this->turboApi->orders->get_order(intval($orderId));
        $paymentMethod = $this->turboApi->payment->get_payment_method(intval($order->payment_method_id));
        $settings      = $this->turboApi->payment->get_payment_settings($paymentMethod->id);
        ...
        try {
            ...
            $return_url = $this->turboApi->config->root_url.'/order/'.$order->url;
    ...
    private function sendSecondReceipt($order, $paymentId)
    {
        ...
        if ($kassaSecondReceipt->sendSecondReceipt()) {
            ...
            $this->turboApi->orders->update_order($order->id, array(
    ...
    {
        $sql   = 'SELECT o.payment_details FROM __orders AS o WHERE o.id='.$this->turboApi->db->escape((int)$orderId);
        $query = $this->turboApi->db->placehold($sql);
        $this->turboApi->db->query($query);

        return $this->turboApi->db->result('payment_details');
    }
    ...
    {
        $this->turboApi->orders->pay($order->id);
        ...
        $this->turboApi->notify->email_order_admin((int)$order->id);
    }
}